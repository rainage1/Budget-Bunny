...
#If user wants to register as a new user, following code asks for their information and writes it all to the data file
  ...
  new = username + ' ' + password + ' none 0 0 0 ' + bankAccount + ' 0 0 0 0'
  fields.append(new.split())
  dataFile.write(new)
  dataFile.close()
  row = findRow(username)

command = menu.menuDisplay(fields[row][8],fields[row][7],fields[row][2])
while command.lower().strip() != 'l':
  ...
  if command.lower().strip() == 'm':
    ...
    if fields[row][2].isnumeric():
      print("\nYou already have an ongoing goal with ${:s} saved after {:d} day(s)".format(fields[row][2], datetime.now().day - int(fields[row][5])))
    ...
    if fields[row][2].isalpha():
      ...
      print("_________________________________________________________________\nHello! Today is day", str(datetime.now().day - int(fields[row][5])+1) , "of your money management plan.\nIt is recommended that you deposit $", ((int(fields[row][2]) - int(fields[row][9])))/(int(fields[row][10].strip()) - (datetime.now().day - int(fields[row][5]))) , "into your saving account today.\n_________________________________________________________________")
      command = menu.menuDisplay(fields[row][8],fields[row][7],fields[row][2])    
    ...
    while str(bankAccount) != fields[row][6]:
      bankAccount = input("Invalid bank number. Enter your bank account number: ")
    transferAccount = input("Transger from chequing      -> Press \"C\"\Transfer from saving        -> Press \"S\"\nQuit                        -> Press \"Q\"\n")
    if transferAccount.lower().split() != 'q':
